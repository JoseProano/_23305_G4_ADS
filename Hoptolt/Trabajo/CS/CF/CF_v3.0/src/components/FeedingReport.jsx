...
import '../styles/FeedingReport.css';
...
const FeedingReport = () => {
    ...
    const generateReportPreview = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/reports/feeding-report', {
                method: 'POST',
                headers: {
    ...
    const downloadPDF = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/reports/feeding-report/pdf', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                ...
                link.href = url;
                link.download = `Reporte_Alimentacion_${new Date().toLocaleDateString('es-ES').replace(/\//g, '-')}.pdf`;
    ...
    return (
        <div className="feeding-report-container">
            <div className="report-header">
                <h2>📊 Generar Reporte de Alimentación</h2>
                <p>Genere reportes de alimentación por raza y período de tiempo</p>
            </div>
            ...
            {!showPreview ? (
                ...
                <div className="preview-section">
                    <div className="preview-header">
                        <h3>📋 Vista Previa del Reporte de Alimentación</h3>
                                ...
                                {reportData && reportData.data && reportData.data.length > 0 ? (
                                    reportData.data.map((row, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{row.raza}</td>
                                            <td className="center">{row.peso}</td>
                                            <td className="center">{row.fechaHora}</td>
                                            <td className="center">{row.heno}</td>
                                            <td className="center">{row.hierba}</td>
                                            <td className="center">{row.balanceado}</td>
