...
const validateRabbitData = async (data) => {
    ...
    if (typeof weight !== 'number' || weight <= 0 || weight > 2.5) {
        errors.push('El peso debe ser un número positivo y no superar los 2.5 kg.');
    }
...
const validateCodeUniqueness = async (code, excludeCode = null) => {
    try {
        ...
        console.log('Validando unicidad:', { code, excludeCode });
        
        // Buscar todos los conejos con este código
        const existingRabbits = await Rabbit.find({ code });
        console.log('Conejos encontrados con código:', existingRabbits.length);
        
        // Si no hay conejos con este código, es válido
        if (existingRabbits.length === 0) {
            console.log('No hay conejos con este código, es válido');
            return { valid: true };
        }
        ...
        if (excludeCode) {
            ...
            console.log('Otros conejos (no excluidos):', otherRabbits.length);
            
            if (otherRabbits.length === 0) {
                console.log('Solo existe el conejo que estamos editando, es válido');
                return { valid: true };
            } else {
                console.log('Existe otro conejo con este código, no es válido');
                return { 
                    valid: false, 
            ...
            console.log('El código ya existe y no estamos editando, no es válido');
            return { 
                valid: false, 
